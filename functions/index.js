...
//  PEYFLEX HELPERS
// ────────────────────────────────────────────────
const PEYFLEX_BASE = process.env.PEYFLEX_BASE_URL || "https://client.peyflex.com.ng/api";
let _peyflexToken = null;
let _peyflexTokenExpiry = 0;

async function getPeyflexToken() {
  // Use token from environment variable directly
  const envToken = process.env.PEYFLEX_TOKEN;
  if (envToken) return envToken;

  // Fallback: login with email/password from Firestore
...
app.post("/peyflex/airtime", async (req, res) => {
  console.log(`[AIRTIME] network=${req.body?.network} amount=${req.body?.amount} phone=${req.body?.mobile_number}`);
  try {
...
app.post("/peyflex/data", async (req, res) => {
  console.log(`[DATA] network=${req.body?.network} plan=${req.body?.plan_code} phone=${req.body?.mobile_number}`);
  try {
...
app.post("/peyflex/cable", async (req, res) => {
  console.log(`[CABLE] provider=${req.body?.identifier} plan=${req.body?.plan} iuc=${req.body?.iuc}`);
  try {
...
app.post("/peyflex/electricity", async (req, res) => {
  console.log(`[ELEC] meter=${req.body?.meter} plan=${req.body?.plan} amount=${req.body?.amount}`);
  try {
